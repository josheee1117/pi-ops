...
- **External BASE Trust Anchor.** `pull_request_target` checks out BASE (`fetch-depth: 0`, `persist-credentials: false`) and treats HEAD as a local git object (`git cat-file`). Same-repository PRs only; missing HEAD objects fail closed. The BASE checker never installs or executes HEAD. v2.0 detector decisions: `PASS` / `LOW_PASS` (exit 0), `HUMAN_REQUIRED` (exit 2), `REJECT` (exit 4), `INTERNAL_ERROR` (exit 1). Only `HUMAN_REQUIRED` goes to GitHub Environment `governance-review`. `REJECT` and `INTERNAL_ERROR` fail closed without Environment. No labels, actors, SHAs, commit messages, or branch names authorize. The required check is the **final** job. Not cryptographic immutability.
