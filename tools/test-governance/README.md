...
- **External BASE Trust Anchor.** `pull_request_target` checks out BASE (`fetch-depth: 0`, `persist-credentials: false`) and treats HEAD as a local git object (`git cat-file`). v1 supports **same-repository PRs only**; missing HEAD objects fail closed. Fork PRs need a separately reviewed acquisition design. The BASE checker never installs or executes HEAD. Detector exit codes stay `0` PASS / `2` REVIEW_REQUIRED / `1` internal error. `REVIEW_REQUIRED` is authorized only by GitHub Environment `governance-review` (required reviewers + `GOVERNANCE_REVIEW_CONFIGURED=true`), not by labels, actors, SHAs, or branch names. The required check must be the **final** job, not detect. Authoritative only after the workflow exists on `main`, PRs are required, the final check is required, and the environment is armed. Repository code cannot stop a privileged owner from direct/force push. Not cryptographic immutability.
