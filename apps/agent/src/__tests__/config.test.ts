...
const CONFIG_KEYS = [
  ...
  'PI_OPS_NOTIFICATION_MAX_RESPONSE_BYTES',
  'PI_OPS_NOTIFICATION_JOB_POLL_INTERVAL_MS',
  'PI_OPS_NOTIFICATION_JOB_MAX_ATTEMPTS',
  'PI_OPS_NOTIFICATION_JOB_BATCH_SIZE',
] as const;
...
describe('loadConfig node-agent registry', () => {
  ...
  it('defaults to FakeReasoner without Pi credentials', () => {
    withConfigEnv({}, () => {
      const config = loadConfig();
